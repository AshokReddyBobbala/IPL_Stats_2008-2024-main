...
st.markdown("<h1 style='color: purple;'>Indian Premier League Stats</h1>", unsafe_allow_html=True)
# st.sidebar.title('Indian Premier League')
st.sidebar.markdown("<h1 style='color: #306998;'>Indian Premier League</h1>", unsafe_allow_html=True)
st.sidebar.markdown("<h3 style='color: #B22222;'>Choose stats</h3>", unsafe_allow_html=True)
...
stadium.fillna({'city': 'Abu Dhabi'}, inplace=True)
stads = []
...
if category == 'Team Stats' :
    ...
    st.markdown("<h2 style='color: #000080;'>Team Stats</h2>", unsafe_allow_html=True)
    ...
    if season != 'Overall' :
        data = data.loc[data['season'].astype(str)==season]
    ...
    if len(data) == 0 :
        ...
        st.write(' ')
        st.markdown("<h2 style='color: #000080;'>Toss and Winning Probability</h2>", unsafe_allow_html=True)
        st.write(' ')
...
if category == 'Stadium Stats' :
    ...
    st.markdown("<h2 style='color: #000080;'>Stadium Stats</h2>", unsafe_allow_html=True)
...
if category == 'Head to Head Matches Stats' :
    ...
    st.markdown("<h2 style='color: #000080;'>Head to Head Matches Stats</h2>", unsafe_allow_html=True)
...
if category == 'Top Run Scorers' :
    ...
    st.markdown("<h2 style='color: #000080;'>Top Run Scorers</h2>", unsafe_allow_html=True)
    chosen = st.slider('Select number of top run scorers',1,100)
    runs = ball_data.groupby('batter')['batsman_runs'].sum()
    bound = ball_data.groupby('batter')['batsman_runs'].value_counts()
    strike = ball_data.loc[ball_data['extras_type'] != 'wides']['batter'].value_counts()
...
if category == 'Top Wicket Takers':
    ...
    st.markdown("<h2 style='color: #000080;'>Top Wicket Takers</h2>", unsafe_allow_html=True)
...
if category == 'Top Man of the Match Award Holders' :
    ...
    st.markdown("<h2 style='color: #000080;'>Top Man of the Match Award Holders</h2>", unsafe_allow_html=True)
    season = st.selectbox('Select Season', year)
    ...
    if season == "Overall" :
        ...
        mmp = matches.loc[matches['season'].astype(str) == season]
